...
const inputFile = "data/global-trending-repositories-500.json";
const outputFile = "data/global-trending-clone-status-500.json";
...
async function existingCheckoutIsReadable(localPath) {
  const head = await run("git", ["-C", localPath, "rev-parse", "--short", "HEAD"], { timeoutMs: 10000 });
  if (head.code !== 0) return { ok: false, head: null };
  const deleted = await run("git", ["-C", localPath, "ls-files", "-d"], { timeoutMs: 10000 });
  if (deleted.code === 0 && deleted.stdout.trim()) return { ok: false, head: head.stdout.trim() || null };
  return { ok: true, head: head.stdout.trim() || null };
}
...
async function cloneRepo(repo, index) {
  ...
  if (existsSync(gitDir)) {
    const existing = await existingCheckoutIsReadable(localPath);
    if (existing.ok) {
      return {
        index,
        name: repo.name,
