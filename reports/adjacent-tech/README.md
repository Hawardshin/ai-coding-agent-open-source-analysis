# Adjacent AI Infrastructure Reports

Generated: 2026-06-18T15:04:44.222Z

Context engineering, RAG, local LLM serving, vector databases, evals, observability, and harness infrastructure.

## 요약

- 조사 단위: `reports/adjacent-tech` 물리 폴더에 모인 보고서 묶음입니다.
- 포함 범위: 4 reports이며, 주요 유형은 infrastructure-report (4)입니다.
- 탐색 방식: 아래 Navigation에서 상위 인덱스로 이동하거나, Related Topic Pages에서 같은 보고서를 주제 기준으로 다시 볼 수 있습니다.

## 총평

이 README는 원본 보고서를 옮기지 않고 폴더 단위로 읽는 입구입니다. 먼저 Recommended Reading Path로 핵심 보고서를 훑고, 필요하면 All Reports in This Folder에서 개별 조사로 내려가는 흐름이 가장 안정적입니다. 관련 주제 페이지 7개와 연결되어 있어 폴더 기준과 주제 기준을 오가며 볼 수 있습니다.

## Navigation

| Entry | Use it for |
| --- | --- |
| [Repository README](../../README.md) | Repo-wide orientation and top-level data/report structure. |
| [Reports Reading Index](../README.md) | Main report navigation, start-here path, topics, and folder map. |
| [Reports by Topic](../by-topic/README.md) | Topic-first navigation across all Markdown reports. |
| [Report Tables](../tables/README.md) | Table-first view and CSV exports. |
| [Repository Insights](../repository-insights/README.md) | Repository-by-repository insights, risks, and next-read links. |
| [Source Deep Dives](../source-deep-dives/README.md) | Topic-wise source-path evidence from local clones. |
| [Artifact Categories](../categories/README.md) | Artifact-level categories across repositories, papers, presentations, and references. |


## Folder Summary

| Field | Value |
| --- | --- |
| Folder | `reports/adjacent-tech` |
| Role | Adjacent stack synthesis and source maps. |
| Reports | 4 |
| Main report types | infrastructure-report (4) |

## Related Topic Pages

| Topic | Reports in this folder |
| --- | ---: |
| [Adjacent AI Infrastructure](../by-topic/adjacent-infrastructure/README.md) | 4 |
| [LLM Wiki, RAG, and Knowledge Bases](../by-topic/llm-wiki/README.md) | 4 |
| [Agent Harness, MCP, and Orchestration](../by-topic/agent-harness/README.md) | 2 |
| [Presentations and Conferences](../by-topic/presentations-conferences/README.md) | 1 |
| [Research Foundations](../by-topic/research-foundations/README.md) | 1 |
| [Source Code Deep Dives](../by-topic/source-code-deep-dives/README.md) | 1 |
| [Start Here](../by-topic/start-here/README.md) | 1 |


## Recommended Reading Path

| Report | Type | Folder | Priority |
| --- | --- | --- | ---: |
| [컨텍스트 엔지니어링, RAG, vLLM/local LLM, 에이전트 하네스 최신 트렌드 분석](01-context-rag-vllm-local-llm-harness-trends.md) | infrastructure-report | adjacent-tech | 157 |
| [컨텍스트/RAG/vLLM/local LLM/하네스 50개 레포지토리 상세 분석](02-repository-analysis-50.md) | infrastructure-report | adjacent-tech | 157 |
| [공식 문서와 소스맵: Context Engineering, RAG, Local LLM, Agent Harness](03-official-docs-and-source-map.md) | infrastructure-report | adjacent-tech | 117 |
| [2026 최신 자료 레이더: 논문, 빅테크 발표, 에이전트/RAG/로컬 LLM 트렌드](04-latest-research-and-industry-radar-2026.md) | infrastructure-report | adjacent-tech | 117 |


## All Reports in This Folder

| Report | Type | Folder | Priority |
| --- | --- | --- | ---: |
| [컨텍스트 엔지니어링, RAG, vLLM/local LLM, 에이전트 하네스 최신 트렌드 분석](01-context-rag-vllm-local-llm-harness-trends.md) | infrastructure-report | adjacent-tech | 157 |
| [컨텍스트/RAG/vLLM/local LLM/하네스 50개 레포지토리 상세 분석](02-repository-analysis-50.md) | infrastructure-report | adjacent-tech | 157 |
| [공식 문서와 소스맵: Context Engineering, RAG, Local LLM, Agent Harness](03-official-docs-and-source-map.md) | infrastructure-report | adjacent-tech | 117 |
| [2026 최신 자료 레이더: 논문, 빅테크 발표, 에이전트/RAG/로컬 LLM 트렌드](04-latest-research-and-industry-radar-2026.md) | infrastructure-report | adjacent-tech | 117 |
