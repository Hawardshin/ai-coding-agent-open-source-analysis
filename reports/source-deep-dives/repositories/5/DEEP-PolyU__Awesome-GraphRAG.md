# DEEP-PolyU/Awesome-GraphRAG 소스 딥다이브
...
- 조사 단위: `sources/DEEP-PolyU__Awesome-GraphRAG` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 7 files, 1 directories, depth score 25, key references 3개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

LLM 위키/RAG/지식베이스 관점에서 retrieval/vector path 구조로 읽힌다. 핵심 소스 근거는 retrieval=figs/rag_vs_graphrag.png, figs/retrieval.png이고, 의존성 단서는 의존성 단서 약함, 검증/운영 단서는 운영 보조 파일은 추가 확인 필요이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 3개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 지식/RAG 패턴 참고이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 항목 | 값 |
| --- | --- |
| 레포 | DEEP-PolyU/Awesome-GraphRAG |
...
| 성숙도 | 안정 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/DEEP-PolyU__Awesome-GraphRAG](../../../../sources/DEEP-PolyU__Awesome-GraphRAG) |
| 기존 보고서 | [reports/llm-wiki/repositories/DEEP-PolyU__Awesome-GraphRAG.md](../../../llm-wiki/repositories/DEEP-PolyU__Awesome-GraphRAG.md) |
...
1. 핵심 참조에서 시작: `figs/rag_vs_graphrag.png`, `figs/retrieval.png`, `README.md`.
2. retrieval/memory/indexing 확인: `figs/rag_vs_graphrag.png`, `figs/retrieval.png`.

## 기존 레포 인사이트

LLM 위키/RAG/지식베이스 관점에서 Awesome GraphRAG A curated list of resources surveys, papers, benchmarks, and opensource projects on graph based retriev. 핵심 구조 신호는 README.md, LICENSE, docs이며, 소스+보고서 근거 수준으로 안정 후보로 읽는 것이 좋습니다.
