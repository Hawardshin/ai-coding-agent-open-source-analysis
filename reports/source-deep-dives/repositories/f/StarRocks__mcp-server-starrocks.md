# StarRocks/mcp-server-starrocks 소스 딥다이브
...
- 조사 단위: `sources/StarRocks__mcp-server-starrocks` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 20 files, 5 directories, depth score 80, key references 10개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

에이전트 하네스/MCP 관점에서 cli-first, api/server, agent/tool runtime 구조로 읽힌다. 핵심 소스 근거는 mcp=src/mcp_server_starrocks/__init__.py, src/mcp_server_starrocks/connection_health_checker.py, src/mcp_server_starrocks/db_client.py이고, 의존성 단서는 mcp, 검증/운영 단서는 test/eval 경로가 보임, CI 워크플로가 보임, 에이전트 지시문 파일이 보임이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 10개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 도구/하네스 패턴 참고이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 항목 | 값 |
| --- | --- |
| 레포 | StarRocks/mcp-server-starrocks |
...
| 성숙도 | 안정 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/StarRocks__mcp-server-starrocks](../../../../sources/StarRocks__mcp-server-starrocks) |
| 기존 보고서 | [reports/global-trending/repositories/StarRocks__mcp-server-starrocks.md](../../../global-trending/repositories/StarRocks__mcp-server-starrocks.md) |
...
| 파일 / 디렉터리 | 20 / 5 |
| 관측 최대 깊이 | 3 |
| 상위 디렉터리 | .github, src, tests |
| 상위 확장자 | .py: 8, .md: 4, (none): 3, .ini: 1, .jpg: 1, .json: 1, .toml: 1, .yml: 1 |
...
| 표면 | Hit 수 | 대표 경로 |
| --- | ---: | --- |
| 테스트/평가 | 3 | [tests/__init__.py](../../../../sources/StarRocks__mcp-server-starrocks/tests/__init__.py)<br>[tests/README.md](../../../../sources/StarRocks__mcp-server-starrocks/tests/README.md)<br>[tests/test_db_client.py](../../../../sources/StarRocks__mcp-server-starrocks/tests/test_db_client.py) |
| CI workflow | 1 | [.github/workflows/publish.yml](../../../../sources/StarRocks__mcp-server-starrocks/.github/workflows/publish.yml) |
| 컨테이너/배포 | 0 | 명확하지 않음 |
| 보안/정책 | 1 | [src/mcp_server_starrocks/secret_resolver.py](../../../../sources/StarRocks__mcp-server-starrocks/src/mcp_server_starrocks/secret_resolver.py) |
| 에이전트 지시문 | 1 | [CLAUDE.md](../../../../sources/StarRocks__mcp-server-starrocks/CLAUDE.md) |
...
1. 핵심 참조에서 시작: `src/mcp_server_starrocks/__init__.py`, `src/mcp_server_starrocks/connection_health_checker.py`, `src/mcp_server_starrocks/db_client.py`.
2. entrypoint를 따라 실행 흐름 확인: `src/mcp_server_starrocks/server.py`.
3. test/eval 파일로 동작 검증: `tests/__init__.py`, `tests/README.md`, `tests/test_db_client.py`.

## 기존 레포 인사이트

에이전트 하네스/MCP 관점에서 StarRocks MCP Model Context Protocol Server. 핵심 구조 신호는 Python, pyproject.toml, README.md, CLAUDE.md, LICENSE, mcp이며, 소스+보고서 근거 수준으로 안정 후보로 읽는 것이 좋습니다.
