...
- 조사 단위: `sources/naver__croco` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 47 files, 8 directories, depth score 45, key references 5개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

일반 AI 오픈소스 관점에서 retrieval/vector path, eval/test harness 구조로 읽힌다. 핵심 소스 근거는 docs=README.MD, stereoflow/README.MD, datasets/habitat_sim/README.MD이고, 의존성 단서는 의존성 단서 약함, 검증/운영 단서는 test/eval 경로가 보임이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 5개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 아키텍처 비교 지점이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 성숙도 | 초기 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/naver__croco](../../../../sources/naver__croco) |
| 기존 보고서 | [reports/korea-trending/repositories/naver__croco.md](../../../korea-trending/repositories/naver__croco.md) |
...
| 파일 / 디렉터리 | 47 / 8 |
| 관측 최대 깊이 | 3 |
| 상위 디렉터리 | assets, datasets, models, stereoflow, utils |
| 상위 확장자 | .py: 32, .md: 4, (none): 3, .ipynb: 2, .png: 2, .cpp: 1, .cu: 1, .jpg: 1, .sh: 1 |
...
| 표면 | Hit 수 | 대표 경로 |
| --- | ---: | --- |
| 테스트/평가 | 1 | [stereoflow/test.py](../../../../sources/naver__croco/stereoflow/test.py) |
...
1. 핵심 참조에서 시작: `README.MD`, `stereoflow/README.MD`, `datasets/habitat_sim/README.MD`.
2. retrieval/memory/indexing 확인: `models/pos_embed.py`.
3. test/eval 파일로 동작 검증: `stereoflow/test.py`.
