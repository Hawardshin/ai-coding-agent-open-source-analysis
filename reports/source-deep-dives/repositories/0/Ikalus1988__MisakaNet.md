# Ikalus1988/MisakaNet Source Deep Dive
...
- 조사 단위: `sources/Ikalus1988__MisakaNet` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 deep dive입니다.
- 포함 범위: 643 files, 75 directories, depth score 120, key references 12개입니다.
- 탐색 방식: Reading Plan을 먼저 보고, Evidence Buckets와 Key Source References의 파일 링크를 따라가면 됩니다.

## 총평

LLM 위키/RAG/지식베이스 관점에서 cli-first, agent/tool runtime, retrieval/vector path 구조로 읽힌다. 핵심 소스 근거는 retrieval=search_knowledge.py, workers/email-register/src/index.js, tests/test_search_knowledge_stdout.py이고, 의존성 단서는 transformers, chroma, 검증/운영 단서는 test/eval 경로가 보임, CI workflow가 보임, 에이전트 지시문 파일이 보임이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 12개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 knowledge/RAG pattern reference이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| Source | [sources/Ikalus1988__MisakaNet](../../../../sources/Ikalus1988__MisakaNet) |
| Existing report | [reports/global-trending/repositories/Ikalus1988__MisakaNet.md](../../../global-trending/repositories/Ikalus1988__MisakaNet.md) |
...
| Field | Value |
| --- | --- |
| Files / directories | 643 / 75 |
| Max observed depth | 4 |
| Top directories | .github, .nodes, .responses, archive, avatars, bench_results, data, docs, hub, lessons, meta, misakanet, promotional, reference, schemas, scripts, tasks, tests, web, workers |
| Top extensions | .md: 311, .json: 121, .py: 92, .png: 39, .yml: 36, (none): 11, .js: 6, .sh: 4, .svg: 3, .txt: 3, .7z: 2, .example: 2 |
...
| Agent instructions | 3 | [AGENTS.md](../../../../sources/Ikalus1988__MisakaNet/AGENTS.md)<br>[CLAUDE.md](../../../../sources/Ikalus1988__MisakaNet/CLAUDE.md)<br>[docs/domains/claude.md](../../../../sources/Ikalus1988__MisakaNet/docs/domains/claude.md) |
...
1. Start from key references: `search_knowledge.py`, `workers/email-register/src/index.js`, `tests/test_search_knowledge_stdout.py`.
2. Trace execution through entrypoints: `workers/email-register/src/index.js`, `misakanet/__main__.py`.
3. Map agent/tool runtime through: `AGENT_GUIDE.md`, `AGENTS.md`, `reference/agent-medici-architecture-analysis-methodology.md`.
4. Inspect retrieval/memory/indexing through: `search_knowledge.py`, `workers/email-register/src/index.js`, `tests/test_search_knowledge_stdout.py`.
5. Verify behavior through test/eval files: `test-nodes.json`, `tests/frontend.test.js`, `tests/test_ci_self_heal.py`.

## Existing Repository Insight

LLM 위키/RAG/지식베이스 관점에서 📚 A zero dependency, git backed micro lesson library for AI Agents to asynchronously share and search verified debuggin. 핵심 구조 신호는 Python, package.json, pyproject.toml, requirements.txt, Makefile, README.md이며, source+report 근거 수준으로 solid 후보로 읽는 것이 좋습니다.
