...
- 조사 단위: `sources/tornikebolokadze1-cyber__awesome-ai-pulse-georgia` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 25 files, 8 directories, depth score 74, key references 10개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

에이전트 하네스/MCP 관점에서 cli-first, agent/tool runtime, retrieval/vector path 구조로 읽힌다. 핵심 소스 근거는 mcp=mcp/.gitignore, mcp/LICENSE, mcp/package-lock.json이고, 의존성 단서는 의존성 단서 약함, 검증/운영 단서는 test/eval 경로가 보임, CI 워크플로가 보임이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 10개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 도구/하네스 패턴 참고이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 항목 | 값 |
| --- | --- |
| 레포 | tornikebolokadze1-cyber/awesome-ai-pulse-georgia |
...
| 성숙도 | 안정 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/tornikebolokadze1-cyber__awesome-ai-pulse-georgia](../../../../sources/tornikebolokadze1-cyber__awesome-ai-pulse-georgia) |
| 기존 보고서 | [reports/global-trending/repositories/tornikebolokadze1-cyber__awesome-ai-pulse-georgia.md](../../../global-trending/repositories/tornikebolokadze1-cyber__awesome-ai-pulse-georgia.md) |
...
| 파일 / 디렉터리 | 25 / 8 |
| 관측 최대 깊이 | 3 |
| 상위 디렉터리 | .github, assets, docs, mcp |
| 상위 확장자 | .ts: 7, .json: 5, (none): 4, .md: 3, .yml: 3, .html: 1, .svg: 1, .yaml: 1 |
...
| 표면 | Hit 수 | 대표 경로 |
| --- | ---: | --- |
| 테스트/평가 | 1 | [mcp/scripts/smoke-test.ts](../../../../sources/tornikebolokadze1-cyber__awesome-ai-pulse-georgia/mcp/scripts/smoke-test.ts) |
| CI workflow | 3 | [.github/workflows/mcp-data-sync.yml](../../../../sources/tornikebolokadze1-cyber__awesome-ai-pulse-georgia/.github/workflows/mcp-data-sync.yml)<br>[.github/workflows/mcp-publish.yml](../../../../sources/tornikebolokadze1-cyber__awesome-ai-pulse-georgia/.github/workflows/mcp-publish.yml)<br>[.github/workflows/mcp-star-refresh.yml](../../../../sources/tornikebolokadze1-cyber__awesome-ai-pulse-georgia/.github/workflows/mcp-star-refresh.yml) |
| 컨테이너/배포 | 0 | 명확하지 않음 |
| 보안/정책 | 0 | 명확하지 않음 |
| 에이전트 지시문 | 0 | 명확하지 않음 |
...
1. 핵심 참조에서 시작: `mcp/.gitignore`, `mcp/LICENSE`, `mcp/package-lock.json`.
2. entrypoint를 따라 실행 흐름 확인: `mcp/src/cli.ts`, `mcp/src/index.ts`.
3. retrieval/memory/indexing 확인: `mcp/src/index.ts`, `docs/index.html`.
4. test/eval 파일로 동작 검증: `mcp/scripts/smoke-test.ts`.

## 기존 레포 인사이트

에이전트 하네스/MCP 관점에서 A curated collection of cutting edge AI agent frameworks, developer tools, and automation resources — by AI Pulse Georgi. 핵심 구조 신호는 TypeScript, README.md, LICENSE, ci, docs, spec-artifacts이며, 소스+보고서 근거 수준으로 안정 후보로 읽는 것이 좋습니다.
