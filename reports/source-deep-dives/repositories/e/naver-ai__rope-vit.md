...
- 조사 단위: `sources/naver-ai__rope-vit` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 136 files, 18 directories, depth score 66, key references 8개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

LLM 위키/RAG/지식베이스 관점에서 cli-first, spec/docs-driven, eval/test harness 구조로 읽힌다. 핵심 소스 근거는 entrypoints=SwinTransformer/main.py, deit/main.py이고, 의존성 단서는 의존성 단서 약함, 검증/운영 단서는 test/eval 경로가 보임이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 8개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 지식/RAG 패턴 참고이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 성숙도 | 초기 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/naver-ai__rope-vit](../../../../sources/naver-ai__rope-vit) |
| 기존 보고서 | [reports/korea-trending/repositories/naver-ai__rope-vit.md](../../../korea-trending/repositories/naver-ai__rope-vit.md) |
...
| 파일 / 디렉터리 | 136 / 18 |
| 관측 최대 깊이 | 4 |
| 상위 디렉터리 | deit, figures, logs, models, self-attn, SwinTransformer |
| 상위 확장자 | .yaml: 52, .py: 44, .txt: 26, .md: 5, (none): 4, .png: 3, .cpp: 1, .cu: 1 |
...
| 표면 | Hit 수 | 대표 경로 |
| --- | ---: | --- |
| 테스트/평가 | 1 | [SwinTransformer/kernels/window_process/unit_test.py](../../../../sources/naver-ai__rope-vit/SwinTransformer/kernels/window_process/unit_test.py) |
...
1. 핵심 참조에서 시작: `SwinTransformer/main.py`, `deit/main.py`, `README.md`.
2. entrypoint를 따라 실행 흐름 확인: `SwinTransformer/main.py`, `deit/main.py`.
3. test/eval 파일로 동작 검증: `SwinTransformer/kernels/window_process/unit_test.py`.

## 기존 레포 인사이트

LLM 위키/RAG/지식베이스 관점에서 ECCV 2024 Official PyTorch implementation of RoPE ViT "Rotary Position Embedding for Vision Transformer". 핵심 구조 신호는 Python, README.md, docs이며, 소스+보고서 근거 수준으로 초기 후보로 읽는 것이 좋습니다.
