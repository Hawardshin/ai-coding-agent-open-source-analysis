...
- 조사 단위: `sources/Samsung__rlottie` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 322 files, 27 directories, depth score 53, key references 5개입니다.
...
| 성숙도 | 안정 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/Samsung__rlottie](../../../../sources/Samsung__rlottie) |
| 기존 보고서 | [reports/korea-trending/repositories/Samsung__rlottie.md](../../../korea-trending/repositories/Samsung__rlottie.md) |
...
| 파일 / 디렉터리 | 322 / 27 |
| 관측 최대 깊이 | 5 |
| 상위 디렉터리 | .Gifs, cmake, example, inc, licenses, packaging, src, test, vs2019 |
| 상위 확장자 | .json: 105, .h: 86, .cpp: 53, .txt: 16, .build: 14, (none): 5, .in: 3, .png: 3, .filters: 2, .gif: 2, .ico: 2, .sh: 2 |
...
| 표면 | Hit 수 | 대표 경로 |
| --- | ---: | --- |
| 테스트/평가 | 15 | [test/CMakeLists.txt](../../../../sources/Samsung__rlottie/test/CMakeLists.txt)<br>[test/meson.build](../../../../sources/Samsung__rlottie/test/meson.build)<br>[test/test_lottieanimation_capi.cpp](../../../../sources/Samsung__rlottie/test/test_lottieanimation_capi.cpp)<br>[test/test_lottieanimation.cpp](../../../../sources/Samsung__rlottie/test/test_lottieanimation.cpp)<br>[test/test_vpath.cpp](../../../../sources/Samsung__rlottie/test/test_vpath.cpp)<br>[test/test_vrect.cpp](../../../../sources/Samsung__rlottie/test/test_vrect.cpp) |
...
1. 핵심 참조에서 시작: `README.md`, `test/CMakeLists.txt`, `test/meson.build`.
2. retrieval/memory/indexing 확인: `src/vector/CMakeLists.txt`, `src/vector/meson.build`, `src/vector/varenaalloc.cpp`.
3. test/eval 파일로 동작 검증: `test/CMakeLists.txt`, `test/meson.build`, `test/test_lottieanimation_capi.cpp`.

## 기존 레포 인사이트

일반 AI 오픈소스 관점에서 A platform independent standalone library that plays Lottie Animation.. 핵심 구조 신호는 C++, README.md, tests, docs이며, 소스+보고서 근거 수준으로 안정 후보로 읽는 것이 좋습니다.
