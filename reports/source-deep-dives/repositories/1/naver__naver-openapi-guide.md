# naver/naver-openapi-guide 소스 딥다이브
...
- 조사 단위: `sources/naver__naver-openapi-guide` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 455 files, 123 directories, depth score 48, key references 5개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

일반 AI 오픈소스 관점에서 eval/test harness 구조로 읽힌다. 핵심 소스 근거는 docs=ko/README.md, ko/service-apis/README.md, ko/service-apis/search/doc/doc.md이고, 의존성 단서는 의존성 단서 약함, 검증/운영 단서는 test/eval 경로가 보임이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 5개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 아키텍처 비교 지점이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 성숙도 | 초기 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/naver__naver-openapi-guide](../../../../sources/naver__naver-openapi-guide) |
| 기존 보고서 | [reports/korea-trending/repositories/naver__naver-openapi-guide.md](../../../korea-trending/repositories/naver__naver-openapi-guide.md) |
...
| 관측 최대 깊이 | 7 |
| 상위 디렉터리 | ko, sample |
| 상위 확장자 | .png: 167, .md: 115, .php: 26, .js: 25, .java: 24, .cs: 23, .py: 23, .gif: 16, .css: 11, .json: 8, .svg: 8, .aspx: 2 |
...
| 표면 | Hit 수 | 대표 경로 |
| --- | ---: | --- |
| 테스트/평가 | 1 | [ko/sdk-tool/tests.md](../../../../sources/naver__naver-openapi-guide/ko/sdk-tool/tests.md) |
...
1. 핵심 참조에서 시작: `ko/README.md`, `ko/service-apis/README.md`, `ko/service-apis/search/doc/doc.md`.
2. test/eval 파일로 동작 검증: `ko/sdk-tool/tests.md`.

## 기존 레포 인사이트

일반 AI 오픈소스 관점에서 설명 메타데이터가 짧습니다. 핵심 구조 신호는 CSS이며, 소스+보고서 근거 수준으로 초기 후보로 읽는 것이 좋습니다.
