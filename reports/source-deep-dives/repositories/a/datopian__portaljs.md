...
- 조사 단위: `sources/datopian__portaljs` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 2,325 files, 347 directories, depth score 122, key references 12개입니다.
...
| 성숙도 | 고신호 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/datopian__portaljs](../../../../sources/datopian__portaljs) |
| 기존 보고서 | [reports/global-trending/repositories/datopian__portaljs.md](../../../global-trending/repositories/datopian__portaljs.md) |
...
| 파일 / 디렉터리 | 2325 / 347 |
| 관측 최대 깊이 | 8 |
| 상위 디렉터리 | .changeset, .claude, .claude-plugin, .github, assets, cloud, examples, packages, scripts, site, tests |
| 상위 확장자 | .md: 534, .html: 392, .png: 354, .json: 266, .tsx: 236, .ts: 113, .webp: 100, .js: 55, .svg: 51, .jpg: 45, (none): 34, .mdown: 31 |
...
1. 핵심 참조에서 시작: `site/public/static/img/cloud-docs/mcp-claude-add-connector.png`, `site/public/static/img/cloud-docs/mcp-claude-connector-url.png`, `site/public/static/img/cloud-docs/mcp-claude-funcs.png`.
2. entrypoint를 따라 실행 흐름 확인: `site/public/static/icons/light/server.json`, `site/public/static/icons/dark/server.json`, `site/public/globe/main.js`.
3. agent/tool runtime 매핑: `AGENTS.md`, `tests/skill-triggering/run-all.sh`, `tests/skill-triggering/run-test.sh`.
4. retrieval/memory/indexing 확인: `site/specs/index.spec.js`, `site/public/static/img/docs/share-embed-graphs.png`, `site/public/static/img/docs/share-embed-tables.png`.
5. test/eval 파일로 동작 검증: `tests/README.md`, `tests/skill-triggering/run-all.sh`, `tests/skill-triggering/run-test.sh`.

## 기존 레포 인사이트

에이전트 하네스/MCP 관점에서 🌀 AI native framework for building data portals. Scaffold a full portal from a brief and load datasets in minutes with . 핵심 구조 신호는 TypeScript, package.json, README.md, AGENTS.md, CLAUDE.md, LICENSE이며, 소스+보고서 근거 수준으로 고신호 후보로 읽는 것이 좋습니다.
