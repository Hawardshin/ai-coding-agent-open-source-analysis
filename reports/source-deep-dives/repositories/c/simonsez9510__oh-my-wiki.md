# simonsez9510/oh-my-wiki 소스 딥다이브
...
- 조사 단위: `sources/simonsez9510__oh-my-wiki` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 29 files, 13 directories, depth score 32, key references 3개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

LLM 위키/RAG/지식베이스 관점에서 agent/tool runtime 구조로 읽힌다. 핵심 소스 근거는 docs=README.md, demo/README.md, demo/_after_example/README.md이고, 의존성 단서는 의존성 단서 약함, 검증/운영 단서는 운영 보조 파일은 추가 확인 필요이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 3개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 지식/RAG 패턴 참고이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 성숙도 | 초기 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/simonsez9510__oh-my-wiki](../../../../sources/simonsez9510__oh-my-wiki) |
| 기존 보고서 | [reports/korea-trending/repositories/simonsez9510__oh-my-wiki.md](../../../korea-trending/repositories/simonsez9510__oh-my-wiki.md) |
...
| 파일 / 디렉터리 | 29 / 13 |
| 관측 최대 깊이 | 5 |
| 상위 디렉터리 | .claude-plugin, demo, skills |
| 상위 확장자 | .md: 20, (none): 3, .base: 2, .json: 2, .ps1: 2 |
...
1. 핵심 참조에서 시작: `README.md`, `demo/README.md`, `demo/_after_example/README.md`.
2. agent/tool runtime 매핑: `skills/bootstrap/SKILL.md`, `skills/bootstrap/templates/dashboard.base`, `skills/bootstrap/templates/lint.base`.

## 기존 레포 인사이트

LLM 위키/RAG/지식베이스 관점에서 빈 옵시디언 vault를 5분 만에 LLM Wiki 시드로 부트스트랩하는 Claude Code 플러그인 원성묵 원장 강의 IP. 핵심 구조 신호는 PowerShell, README.md, LICENSE, docs이며, 소스+보고서 근거 수준으로 초기 후보로 읽는 것이 좋습니다.
