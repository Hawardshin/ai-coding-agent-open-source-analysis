...
- 조사 단위: `sources/PlateerLab__document-adapter` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 52 files, 8 directories, depth score 62, key references 6개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

에이전트 하네스/MCP 관점에서 cli-first, agent/tool runtime, eval/test harness 구조로 읽힌다. 핵심 소스 근거는 mcp=document_adapter/mcp_server.py이고, 의존성 단서는 anthropic, claude, mcp, 검증/운영 단서는 test/eval 경로가 보임, CI 워크플로가 보임이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 6개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 도구/하네스 패턴 참고이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 항목 | 값 |
| --- | --- |
| 레포 | PlateerLab/document-adapter |
...
| 성숙도 | 안정 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/PlateerLab__document-adapter](../../../../sources/PlateerLab__document-adapter) |
| 기존 보고서 | [reports/korea-trending/repositories/PlateerLab__document-adapter.md](../../../korea-trending/repositories/PlateerLab__document-adapter.md) |
...
| 파일 / 디렉터리 | 52 / 8 |
| 관측 최대 깊이 | 3 |
| 상위 디렉터리 | .github, document_adapter, examples, scripts, tests |
| 상위 확장자 | .py: 45, (none): 3, .md: 2, .toml: 1, .yml: 1 |
...
| 표면 | Hit 수 | 대표 경로 |
| --- | ---: | --- |
| 테스트/평가 | 9 | [tests/test_eval_harness.py](../../../../sources/PlateerLab__document-adapter/tests/test_eval_harness.py)<br>[tests/test_scenarios.py](../../../../sources/PlateerLab__document-adapter/tests/test_scenarios.py)<br>[tests/test_smoke.py](../../../../sources/PlateerLab__document-adapter/tests/test_smoke.py)<br>[examples/eval_run.py](../../../../sources/PlateerLab__document-adapter/examples/eval_run.py)<br>[document_adapter/eval/__init__.py](../../../../sources/PlateerLab__document-adapter/document_adapter/eval/__init__.py)<br>[document_adapter/eval/fixtures.py](../../../../sources/PlateerLab__document-adapter/document_adapter/eval/fixtures.py) |
| CI workflow | 1 | [.github/workflows/tests.yml](../../../../sources/PlateerLab__document-adapter/.github/workflows/tests.yml) |
| 컨테이너/배포 | 0 | 명확하지 않음 |
| 보안/정책 | 0 | 명확하지 않음 |
| 에이전트 지시문 | 0 | 명확하지 않음 |
...
1. 핵심 참조에서 시작: `document_adapter/mcp_server.py`, `document_adapter/tools.py`, `pyproject.toml`.
2. agent/tool runtime 매핑: `document_adapter/tools.py`.
3. test/eval 파일로 동작 검증: `tests/test_eval_harness.py`, `tests/test_scenarios.py`, `tests/test_smoke.py`.

## 기존 레포 인사이트

에이전트 하네스/MCP 관점에서 LLM이 DOCX/PPTX/HWPX 문서를 직접 편집할 수 있게 해주는 통합 어댑터 + MCP 서버. Claude Desktop / Claude Code / Anthropic API Tool Use 호환. pip i. 핵심 구조 신호는 Python, pyproject.toml, README.md, LICENSE, anthropic, mcp이며, 소스+보고서 근거 수준으로 안정 후보로 읽는 것이 좋습니다.
