...
- 조사 단위: `sources/bripin123__rag-memory-epf-mcp` 로컬 클론을 실제 파일 트리 기준으로 분석한 레포별 딥다이브입니다.
- 포함 범위: 40 files, 16 directories, depth score 81, key references 11개입니다.
- 탐색 방식: 읽기 계획을 먼저 보고, 근거 bucket과 핵심 소스 참조의 파일 링크를 따라가면 됩니다.

## 총평

에이전트 하네스/MCP 관점에서 cli-first, agent/tool runtime, retrieval/vector path 구조로 읽힌다. 핵심 소스 근거는 mcp=src/tools/tool-registry.ts, specs/changes/phase3-mcp-sdk/proposal.md, specs/changes/phase3-mcp-sdk/tasks.md이고, 의존성 단서는 modelcontextprotocol, transformers, 검증/운영 단서는 test/eval 경로가 보임이다. 이 판단은 README 메타데이터가 아니라 로컬 소스의 11개 파일 경로를 직접 스캔해야 확인된다. 기존 레포 평가 관점은 도구/하네스 패턴 참고이며, 이 문서는 README/메타데이터가 아니라 실제 소스 경로를 기준으로 후속 확인 지점을 분리합니다.
...
| 항목 | 값 |
| --- | --- |
| 레포 | bripin123/rag-memory-epf-mcp |
...
| 성숙도 | 초기 |
| 근거 수준 | 소스+보고서 |
| 소스 | [sources/bripin123__rag-memory-epf-mcp](../../../../sources/bripin123__rag-memory-epf-mcp) |
| 기존 보고서 | [reports/korea-trending/repositories/bripin123__rag-memory-epf-mcp.md](../../../korea-trending/repositories/bripin123__rag-memory-epf-mcp.md) |
...
| 파일 / 디렉터리 | 40 / 16 |
| 관측 최대 깊이 | 4 |
| 상위 디렉터리 | docs, specs, src, test |
| 상위 확장자 | .md: 15, .ts: 11, .mjs: 7, .json: 3, (none): 3, .yml: 1 |
...
| 표면 | Hit 수 | 대표 경로 |
| --- | ---: | --- |
| 테스트/평가 | 8 | [SPEC-syncDocumentFromFile.md](../../../../sources/bripin123__rag-memory-epf-mcp/SPEC-syncDocumentFromFile.md)<br>[test/chunk-invariants.test.mjs](../../../../sources/bripin123__rag-memory-epf-mcp/test/chunk-invariants.test.mjs)<br>[test/dedup.test.mjs](../../../../sources/bripin123__rag-memory-epf-mcp/test/dedup.test.mjs)<br>[test/engine-smoke.test.mjs](../../../../sources/bripin123__rag-memory-epf-mcp/test/engine-smoke.test.mjs)<br>[test/launch-smoke.test.mjs](../../../../sources/bripin123__rag-memory-epf-mcp/test/launch-smoke.test.mjs)<br>[test/search-degradation.test.mjs](../../../../sources/bripin123__rag-memory-epf-mcp/test/search-degradation.test.mjs) |
...
1. 핵심 참조에서 시작: `src/tools/tool-registry.ts`, `specs/changes/phase3-mcp-sdk/proposal.md`, `specs/changes/phase3-mcp-sdk/tasks.md`.
2. agent/tool runtime 매핑: `src/tools/graph-analytics-tools.ts`, `src/tools/graph-query-tools.ts`, `src/tools/knowledge-graph-tools.ts`.
3. retrieval/memory/indexing 확인: `index.ts`, `test/chunk-invariants.test.mjs`, `src/tools/graph-analytics-tools.ts`.
4. test/eval 파일로 동작 검증: `SPEC-syncDocumentFromFile.md`, `test/chunk-invariants.test.mjs`, `test/dedup.test.mjs`.

## 기존 레포 인사이트

에이전트 하네스/MCP 관점에서 Project local RAG memory MCP server — knowledge graph + multilingual vector + FTS5 in a single SQLite file. Per project . 핵심 구조 신호는 JavaScript, package.json, README.md, LICENSE, modelcontextprotocol, transformers이며, 소스+보고서 근거 수준으로 초기 후보로 읽는 것이 좋습니다.
